...
async def root_status():
    return {"ok": True, "service": "VYLNAX AI Backend", "status": "online"}
...
def _build_system_prompt(patient_name, meds, allergies=None):
    ...
    return (
        "Du bist der VYLNAX PRO KI-Medikamenten-Assistent in einer MedTech-App. "
        "Antworte kurz, klar, freundlich und in der Sprache des Nutzers (Deutsch, Bosnisch oder Englisch). "
...
async def assistant_chat(body: ChatRequest, user=Depends(get_current_user)):
    ...
    # Return the AI answer even if saving chat history fails.
    try:
        ...
        logger.warning("Chat history could not be saved: %s", exc)
