...
def _build_system_prompt(patient_name, meds, lang="de"):
    med_lines = "\n".join(
        [f"- {m['name']} {m['dosage']} ({m['form']}), Zeiten: {', '.join(m.get('times', []))}" for m in meds]
    ) or "Noch keine Medikamente hinterlegt."
    return (
        "Du bist der VYLNAX PRO KI-Medikamenten-Assistent, ein freundlicher, professioneller "
        "Gesundheitsassistent innerhalb einer MedTech-App für sichere Medikamenteneinnahme. "
        f"Der aktuell betreute Patient heißt {patient_name}. Aktuelle Medikamente:\n{med_lines}\n\n"
        "Deine Aufgaben: allgemeine Informationen zu Medikamenten, Einnahmezeiten, möglichen "
        "Wechselwirkungen (allgemein), Nebenwirkungen und zur Nutzung der App geben. "
        "WICHTIG: Du ersetzt KEINEN Arzt oder Apotheker. Weise bei medizinischen Entscheidungen, "
        "Dosisänderungen oder Notfällen immer darauf hin, ärztlichen Rat einzuholen bzw. bei "
        "Notfällen 112 zu rufen. Antworte kurz, klar und empathisch. Antworte in der Sprache des Nutzers "
        "(Deutsch, Englisch oder Bosnisch)."
    )
...
async def assistant_chat(body: ChatRequest, user=Depends(get_current_user)):
    meds = []
    patient_name = user.get("name", "der Patient")
    if body.patient_id:
        p = await db.patients.find_one({"id": body.patient_id, "owner_id": user["user_id"]}, {"_id": 0})
        if p:
            patient_name = p["name"]
            meds = await db.medications.find({"patient_id": body.patient_id}, {"_id": 0}).to_list(100)
    med_lines = ", ".join([f"{m['name']} {m['dosage']}" for m in meds]) or "keine Medikamente"
    msg = body.message.lower()
    if "wechsel" in msg or "interaktion" in msg:
        reply = f"Ich kann Basis-Regeln prüfen. Für {patient_name} sind aktuell hinterlegt: {med_lines}. Öffne 'Sicherheit', um Wechselwirkungen und Allergien zu sehen. Bei medizinischen Entscheidungen bitte Arzt/Apotheke fragen."
    elif "hallo" in msg or "zdravo" in msg:
        reply = f"Hallo! Ich bin der lokale VYLNAX PRO Assistent. Ich kann ohne Emergent-AI Basisinfos anzeigen. Patient: {patient_name}."
    else:
        reply = f"Lokaler Assistent aktiv. Deine Nachricht war: '{body.message}'. Aktuelle Medikamente: {med_lines}. Hinweis: KI/OCR wurde lokal deaktiviert, damit die App ohne Emergent-Modul läuft."
    now = now_utc().isoformat()
    await db.chat_messages.insert_many([
        {"id": uid("msg"), "user_id": user["user_id"], "role": "user", "content": body.message, "created_at": now},
        {"id": uid("msg"), "user_id": user["user_id"], "role": "assistant", "content": reply, "created_at": now_utc().isoformat()},
    ])
    return {"reply": reply}
